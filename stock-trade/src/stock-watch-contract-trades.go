...
var (
	...
	QueURL         = os.Getenv("STOCK_CONTRACT_TRADE_QUE")
	QueName        = os.Getenv("STOCK_CONTRACT_TRADE_QUE_NAME")
	AwsRegion      = os.Getenv("AWS_REGION")
	MaxMessages, _ = strconv.ParseInt(os.Getenv("STOCK_CONTRACT_TRADE_MAX_MESSAGES"), 10, 64)
	PollingTime, _ = strconv.ParseInt(os.Getenv("STOCK_CONTRACT_TRADE_POLLING_TIME"), 10, 64)
)
...
func GetMessage() error {
	...
	for _, msg := range resp.Messages {
		// メッセージが取得できたら約定済み取引を登録する
		fmt.Println(*msg.Body)
		if err := DeleteMessage(subctx, msg); err != nil {
			fmt.Println(err)
		}
		subfunc.UpdateContractTrades(subctx, *msg.Body)
	}
	subseg.Close(nil)
	seg.Close(nil)
